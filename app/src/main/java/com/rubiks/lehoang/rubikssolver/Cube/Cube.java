...
public class Cube {
    ...
    public Cube(byte[] corners, byte[] edges){
        this.edges = edges;
        this.corners = corners;
    }

    public void resetStateTo(Cube cube){
        System.arraycopy(cube.corners, 0, corners, 0 ,corners.length);
        System.arraycopy(cube.edges, 0, edges, 0 , edges.length);
    }

    public void resetStateTo(byte[] cornersBuffer, byte[] edgesBuffer){
        System.arraycopy(cornersBuffer, 0, corners, 0 ,corners.length);
        System.arraycopy(edgesBuffer, 0, edges, 0 , edges.length);
    }

    public void copyCornersInto(byte[] cornerBuffer){
        if(cornerBuffer != null){
            System.arraycopy(corners, 0, cornerBuffer, 0, corners.length);
        }
    }

    public void copyEdgesInto(byte[] edgeBuffer){
        if(edgeBuffer != null){
            System.arraycopy(edges, 0, edgeBuffer, 0, edges.length);
        }
    }
     ...
     * @return
     */
    public static String toKociemba(Cube cube) {
    ...
    public void performSequence(String sequence) throws InvalidMoveException{
        List<Integer> moves = convertToList(sequence);
    ...
    public static List<Integer> convertToList(String sequence) throws InvalidMoveException {
        List<Integer> moves = new ArrayList<Integer>();
        for(int i = sequence.length()-1; i >=0; i--){
            switch(sequence.charAt(i)){
                case '3':
                case'\'':
                    //Account for both types of notation
                    i--;
                    if (i < 0) throw new InvalidMoveException("Cannot start with number");
                    moves.add(0, getMove(sequence.charAt(i))+2);
                    break;
                case '2':
                    i--;
                    if (i < 0) throw new InvalidMoveException("Cannot start with number");
                    moves.add(0,getMove(sequence.charAt(i))+1);
                    break;
                case '1':
                    break;
                case 'R':
                ...
                case 'B':
                case 'D':
                    moves.add(0, getMove(sequence.charAt(i)));
                    break;
                default:
                    throw new InvalidMoveException(sequence + "| index " + i);
            }
        }
        return moves;
    }
